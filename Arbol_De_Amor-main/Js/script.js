...
fetch('Img/simple-Hearts-Tree.svg')
  .then(res => res.text())
  .then(svgText => {
    ...
    const allPaths = Array.from(svg.querySelectorAll('path, rect, polygon, line, circle, ellipse'));
...
function showDedicationText() { //seguidores
  ...
  if (!text) {
    text = `Feliz San Valentin, Mi princesa Hermosa 🧡\n\nAmor, te amo demasiado.\nEres lo mejor que me ha pasado y cada día a tu lado es un regalo que agradezco con el corazón. Te amo muchísimo, porque llegaste a mi vida para darle color a mis días, luz a mis pensamientos y felicidad a mi alma.\n\nNo hay un solo momento en el que no agradezca tenerte. 💖`;  } else {
...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Juanito 🧡\n08/06/2023";
...
function showCountdown() {
  ...
  let startDate = startParam ? new Date(startParam + 'T00:00:00') : new Date('2023-06-08T00:00:00'); 
  let eventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2026-06-08T00:00:00');
